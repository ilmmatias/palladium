...
|---|---|---|---|---|
| Build | C23 CMake/Ninja, Clang/LLD PE targets, compiler-rt bootstrap, Release CI image plus fixed TCG smoke, Python host tests | One accepted `amd64` architecture; no C target tests; local compiler-rt/firmware/tool paths vary | Hosted lane covers one TCG/OVMF profile, not hardware or SMP | Single architecture is current scope; the M0 evidence lane is foundational rather than broad verification |
| UEFI loader | GOP, ACPI, configuration, PE imports/relocations, loader page map, boot drivers, exact loader-block v7 validation | Ordinal imports unsupported; relocation default is not a hard rejection; fixed KDNET discovery; no ARM64 | Malformed PE/config arithmetic and mandatory graphics remain | PE/UEFI custom loader is intentional; unchecked/fixed assumptions are debt |
...
| Python debugger | shared strict-v0 codec/session/command core, curses TUI, headless text/JSONL, memory/disassembly/port reads and export | no register/context, continue/step, reconnect or v1 session semantics | kernel path is not covered by the fake target; interactive TUI breadth remains manual | TUI is useful; v0 is deliberately temporary while the shared frontend boundary should remain |
...
| Diagnostics/tests | framebuffer/KD messages, private polling amd64 serial sink, panic stack trace, disposable QEMU evidence and Python host tests | no structured kernel events, C/AML unit harness, fuzz corpus or SMP/hardware gate | proprietary KD module cannot be bundled; one smoke marker exposes suspect MM accounting | M0 removes the largest boot-observability bottleneck; verification breadth remains debt |
...
Implementation and local qualification are recorded in [the M0 evidence-plane notes](../M0EvidencePlane.md).
