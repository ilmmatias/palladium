...
then
    ...
    qemu-system-x86_64 -enable-kvm -M smm=off -cpu host -smp $(nproc) -drive file=build.target/fat32.img,index=0,media=disk,format=raw -no-reboot $2 1>/dev/null
elif [ "$1" == "exfat" ]
then
    sudo rm -rf /mnt/mount
    sudo mkdir -p /mnt/mount
    ...
    echo "[5/5] Running emulator"
    qemu-system-x86_64 -enable-kvm -M smm=off -cpu host -smp $(nproc) -drive file=build.target/exfat.img,index=0,media=disk,format=raw -no-reboot $2 1>/dev/null
elif [ "$1" == "ntfs" ]
then
    sudo rm -rf /mnt/mount
    sudo mkdir -p /mnt/mount
    ...
    echo "[5/5] Running emulator"
    qemu-system-x86_64 -enable-kvm -M smm=off -cpu host -smp $(nproc) -drive file=build.target/ntfs.img,index=0,media=disk,format=raw -no-reboot $2 1>/dev/null
else
    mkisofs -iso-level 2 -R -b Boot/iso9660boot.com -no-emul-boot -o build.target/iso9660.iso _root 1>/dev/null 2>&1
    echo "[5/5] Running emulator"
    qemu-system-x86_64 -enable-kvm -M smm=off -cpu host -smp $(nproc) -cdrom build.target/iso9660.iso -no-reboot $2 1>/dev/null
fi
