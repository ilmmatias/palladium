...
This Python 3 package is the host TUI/client for the kernel's custom versioned debugger protocol.
...
- `codec.py` constants and little-endian `struct` formats mirror
  ...
  target support. The 48-byte PDKD v1 envelope is strict; serial framing is owned by
  `transport.py` and is not part of the protocol codec.
...
- `transport.py` owns peer-validated UDP and COBS/CRC32C serial I/O, `session.py` owns handshake
  and request state, `commands.py` parses typed commands, and `events.py` is the frontend boundary.
  `interface.py` owns curses while `headless.py` renders text/JSONL. Do not fold wire behavior
  into presentation.
...
Host tests under `tests` use synchronized fake datagram and serial targets. They cover the Python
client but not the kernel's raw Ethernet/KD extension path. Also connect to a debugger-enabled QEMU
boot when kernel wire behavior changes, and never commit generated `__pycache__` data.
